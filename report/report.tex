...
The results are shown in figures \ref{fig:colourfulsub} to \ref{fig:simplediff}. Each resulting subsampled YCbCr image appears to have pink and green hues. Since these exist at the extremes of the YCbCr range of colours, it's likely these are due to incorrect conversions to RGB. Ignoring this, it is still possible to see the effects of subsampling.

In the subsampled images of figure \ref{fig:colorful}, the macroblocks are fairly visible along the edges of objects. Thin outlines of objects that existed in the middle of a cell are thickened due to the averaging of the cells. It is harder to notice shapes in the chromatic images as outlines are less well defined. The cell blocks are especially apparent when the difference between the original and subsampled image are shown (Figure \ref{fig:colourfulsub}). On the other hand, the blue lantern at the bottom right with the pattern of circles does not appear to suffer from aliasing.
...
\begin{figure}[ht]
  \includegraphics[width=\linewidth]{images/task3/colourfuldiff.png}
  \caption{Difference between Figure \ref{fig:colorful} and Figure \ref{fig:colourfulsub}.}
...
\begin{figure}[ht]
  ...
  \label{fig:colorfulyhfspatial}
\end{figure}

The DCT plots of the vector image show more focused frequencies. Most of the energy is focused in the top left, corresponding to low spatial frequency components. This is likely due to the abundance of long lines and no textures. The large triangle that is present in the DCT of all three channels is likely responsible for producing the curved and diagonal lines in the image, as it would require a gradual change in the coeffiecients used to move the maxima responsible for generating the lines. The Cr frequency image (Figure \ref{fig:drawingcrspatial}) shows more triangles than the Cb image (Figure \ref{fig:drawingcbspatial}), which would correspond to there being more shapes with exclusively red chromaticity than blue.

The plot of the highpass filtered image supports this, as the curves are very clear. The lowpass image contains most of the image, due to the large amount of untextured blank space. However, the highpass image shows the curves very brightly. The ripples from the cosine waves that would be canceled out by the low frequency components are visible as streaks from each shape. 
...
\begin{figure}[ht]
  ...
  \label{fig:drawingyhfspatial}
\end{figure}

For the simple image, the concentration of frequencies is almost completely in the low end. The lowpass image shows all white, whilst the high pass image shows the textures in the snow, the dogs coat, and the falling snowflakes. This image would be easily compressed compared to the others as it has the least high frequency components. The vector image could also be compressed without affecting the image quality too much, but aliasing would be present in the curves if overdone.
...
In order to determine the heart rate of the person in the given video, the following steps were taken:

\begin{enumerate}
	\item For each frame of the video, produce the average luminance.
	\item Produce a time value for each average luminance value using the frame rate.
	\item Normalise the luminance signal to oscillate around 0.
	\item Perform a fast fourier transform on the average values.
	\item Plot the time and frequency domain of the average luminance.
\end{enumerate}

The luminance of each frame would vary with the flow of blood under the skin. By taking the average luminance of each frame, the signal is condensed to two dimensions. Since the luminance values are all positive, it was reduced to oscillate around zero by subtracting the average of the max and min values. The FFT could then be performed to transform the signal to the frequency domain, which when plotted would show the frequency of the change in luminance, i.e. the heartrate. 

The nyquist limit was calculated from half of the frame rate of the video. Since a healthy human heart rate even during exercise only peaks at around 190bpm (~3Hz), the videos frame rate of 30Hz would not cause aliasing of the signal. 

The result can be seen in figure \ref{fig:heart}. From the frequency domain plot, we can determine with some confidence that the heart rate is around 1Hz, or in more common units, 60bpm. This aligns well with the average resting heartbeat. Measuring the time between peaks in the frequency plot also produces a period of ~1s therefore 1Hz. 

Hospitals and medical professionals often use a device that uses a similar mechanism to measure heart rates. A clip applied to the thumb shines an LED through the thumb, and a sensor on the opposite side detects the luminance over time in order to determine the heart rate of the wearer. Since oxygenated blood is lighter than deoxygenated blood, the red chromaticity could also possibly be used to determine the blood oxygen level of the wearer, providing more crucial information. 

\begin{figure}[ht]
  \includegraphics[width=\linewidth]{images/task5/Frequency.png}
  \caption{Time and Frequency domain plots of the average luminance of the given video.}
  \label{fig:heart}
\end{figure}
